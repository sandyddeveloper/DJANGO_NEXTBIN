...
class APILog(BaseModel):
    ...
    method = models.CharField(
        max_length=10, choices=METHOD_CHOICES, help_text='HTTP method.'
    )
    ...
    ip_address = models.GenericIPAddressField(
        null=True, blank=True, help_text='Client IP address (IPv4 or IPv6).'
    )
...
class UserActivityLog(models.Model):
    ...
    def __str__(self):
        # Using string profile_code since CustomUser doesn't have a standard username field
        username = getattr(self.user, 'profile_code', str(self.user))
        return f"{username} — {self.action} at {self.created_at:%Y-%m-%d %H:%M}"
...
class SystemSettings(models.Model):
    ...
    key = models.CharField(
        max_length=255, unique=True, help_text='Unique setting key, e.g. `SITE_NAME`.'
    )
    value = models.TextField(help_text='Setting value.')
    description = models.TextField(
        blank=True, help_text='Human-readable explanation of the setting.'
    )
