...
def test_reset_password_success(api_client, verified_user):
    ...
    response = api_client.post(url_reset, {
    ...
    assert not EmailOTP.objects.filter(email=verified_user.email, purpose=OtpPurpose.PASSWORD_RESET).exists()
...
def test_reset_pin_success(api_client, verified_user):
    ...
    # Verify OTP record was deleted
    assert not EmailOTP.objects.filter(email=verified_user.email, purpose=OtpPurpose.PIN_RESET).exists()
